...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  if (cell.enabled) {
    ...
    for (NSDictionary * availableDay in _calendarDays)
    {
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
        NSDate *dateFromString = [[NSDate alloc] init];
          
        dateFromString = [dateFormatter dateFromString: availableDay[@"day"]];
          
        if ([date compare: dateFromString] == NSOrderedSame)
        {
            int dayTime = [availableDay[@"dayTime"] intValue];
            cell.status = dayTime;
        }
      }
  }
